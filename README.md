# Finance API

[![CI](https://github.com/Kazakbay/finance-api/actions/workflows/ci.yml/badge.svg)](https://github.com/Kazakbay/finance-api/actions/workflows/ci.yml)
[![Python](https://img.shields.io/badge/python-3.11-blue.svg)](https://www.python.org/downloads/)
[![FastAPI](https://img.shields.io/badge/FastAPI-0.136-009688.svg)](https://fastapi.tiangolo.com/)
[![Render](https://img.shields.io/badge/Render-deployed-46E3B7?logo=render&logoColor=white)](https://finance-api-hbxy.onrender.com/docs)

🚀 **Live demo:** [finance-api-hbxy.onrender.com/docs](https://finance-api-hbxy.onrender.com/docs)

REST API для учёта личных финансовых транзакций. Pet-проект, написанный для практики backend-разработки на Python.

> ⚠️ Приложение задеплоено на Free-плане Render и «засыпает» после 15 минут простоя. Первый запрос может занять 30–60 секунд, пока контейнер просыпается.

## Стек

- **Python 3.11** + **FastAPI** — веб-фреймворк
- **SQLAlchemy** — ORM
- **SQLite** — база данных
- **Pytest** — юнит-тесты
- **Docker** + **Docker Compose** — контейнеризация
- **GitHub Actions** — CI: автозапуск тестов и линтера на каждый push
- **Render** — CD: автодеплой при пуше в `main`
- **Ruff** — линтер и форматтер

## Возможности

- CRUD-операции для финансовых транзакций (сумма, категория)
- JWT-аутентификация (регистрация, логин, защищённые эндпоинты)
- Изоляция данных между пользователями
- Пагинация и фильтрация транзакций по категории
- Pydantic-схемы для валидации запросов и ответов
- Автогенерируемая документация (Swagger UI / ReDoc)
- Покрытие юнит-тестами
- Конфигурация через переменные окружения
- Запуск одной командой через Docker Compose
- CI/CD-пайплайн от пуша до прода

## Запуск локально

### Через Python

```bash
# 1. Клонировать репозиторий
git clone https://github.com/Kazakbay/finance-api.git
cd finance-api

# 2. Создать виртуальное окружение
python -m venv .venv

# 3. Активировать (Windows)
.venv\Scripts\activate

# Активировать (Linux / macOS)
source .venv/bin/activate

# 4. Установить зависимости
pip install -r requirements.txt

# 5. Создать .env файл
cp .env.example .env
# Открыть .env и заполнить SECRET_KEY

# 6. Запустить сервер
uvicorn main:app --reload
```

API будет доступен на `http://localhost:8000`

### Через Docker Compose

```bash
docker-compose up --build
```

## Документация API

После запуска открой в браузере:

- Swagger UI: `http://localhost:8000/docs`
- ReDoc: `http://localhost:8000/redoc`

Или в проде: [finance-api-hbxy.onrender.com/docs](https://finance-api-hbxy.onrender.com/docs)

## Тесты

```bash
pytest -v
```

## Конфигурация

Приложение читает настройки из переменных окружения. Скопируй `.env.example` в `.env` и заполни значения.

| Переменная     | Значение по умолчанию    | Описание                                     |
|----------------|--------------------------|----------------------------------------------|
| `DATABASE_URL` | `sqlite:///./finance.db` | Строка подключения к БД                      |
| `SECRET_KEY`   | —                        | Секретный ключ для подписи JWT-токенов       |
| `PORT`         | `8000`                   | Порт, на котором слушает сервер (для Render) |

## Структура проекта

```
finance-api/
├── .github/workflows/    # GitHub Actions (CI)
├── main.py               # точка входа FastAPI, эндпоинты
├── database.py           # настройка SQLAlchemy и модели
├── auth.py               # JWT-аутентификация, хеширование паролей
├── schemas.py            # Pydantic-схемы для валидации
├── dependencies.py       # общие зависимости (get_db)
├── test_main.py          # юнит-тесты
├── Dockerfile            # сборка контейнера с API
├── docker-compose.yaml   # оркестрация контейнеров
├── requirements.txt      # Python-зависимости
├── .env.example          # пример переменных окружения
├── .dockerignore
├── .gitignore
└── README.md
```

## CI/CD

**CI (GitHub Actions)** — на каждый push в `main` автоматически:

1. Проверяет код линтером (`ruff check`)
2. Проверяет форматирование (`ruff format --check`)
3. Запускает все тесты (`pytest`)
4. Собирает Docker-образ

**CD (Render)** — после успешного пуша Render автоматически собирает Docker-образ и передеплоивает приложение в прод.

Статус последнего запуска CI отображается в бейдже в верхней части README.

## Автор

Нурдаулет — студент Astana IT University, изучаю backend-разработку на Python.

- GitHub: [@Kazakbay](https://github.com/Kazakbay)